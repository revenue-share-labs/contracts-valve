...
async function deployRSCValve(
    ...
    const XLARSCValveFactory = await ethers.getContractFactory("XLARSCValveFactory");
    ...
    const XLARevenueShareContract = await ethers.getContractFactory("XLARSCValve");
    const xlaRSCValve = await XLARevenueShareContract.attach(revenueShareContractAddress);
    return xlaRSCValve;
}
...
describe("XLA RSC Valve tests", function () {
  ...
  it("Should set base attrs correctly", async () => {
    ...
    expect(await xlaRSCValve.minAutoDistributionAmount()).to.be.equal(ethers.utils.parseEther("1"));
    await xlaRSCValve.setMinAutoDistributionAmount(ethers.utils.parseEther("2"));
    expect(await xlaRSCValve.minAutoDistributionAmount()).to.be.equal(ethers.utils.parseEther("2"));

    await expect(
        xlaRSCValve.connect(addr1).setMinAutoDistributionAmount(ethers.utils.parseEther("2"))
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });

  it("Should set recipients correctly", async() => {
    await expect(
        xlaRSCValve.connect(addr3).setRecipients(
    ...
    expect(await xlaRSCValve.recipientsPercentage(addr1.address)).to.be.equal(2000000);
    expect(await xlaRSCValve.recipientsPercentage(addr3.address)).to.be.equal(5000000);
    expect(await xlaRSCValve.recipientsPercentage(addr4.address)).to.be.equal(3000000);
    ...
    expect(await xlaRSCValve.recipientsPercentage(addr5.address)).to.be.equal(2000000);
    expect(await xlaRSCValve.recipientsPercentage(addr4.address)).to.be.equal(2000000);
    expect(await xlaRSCValve.recipientsPercentage(addr3.address)).to.be.equal(3000000);
    expect(await xlaRSCValve.recipientsPercentage(addr1.address)).to.be.equal(3000000);
    expect(await xlaRSCValve.numberOfRecipients()).to.be.equal(4);

    await xlaRSCValve.setController(constants.ZERO_ADDRESS);
  ...
  it("Should set recipients correctly and set immutable recipients", async() => {
    await expect(
      xlaRSCValve.connect(addr3).setRecipientsExt(
        [addr1.address, addr3.address, addr4.address],
        [2000000, 5000000, 3000000],
      )
    ).to.be.revertedWithCustomError(xlaRSCValve, "OnlyControllerError");
  ...
  it("Should redistribute eth correctly", async() => {
    ...
    expect(addr1BalanceAfter).to.be.equal(addr1BalanceBefore + ethers.utils.parseEther("40").toBigInt());
    expect(addr2BalanceAfter).to.be.equal(addr2BalanceBefore + ethers.utils.parseEther("10").toBigInt());
    ...
    ).to.be.equal(addr1BalanceAfter + ethers.utils.parseEther("0.4").toBigInt());
    expect(
        (await ethers.provider.getBalance(addr2.address)).toBigInt()
    ).to.be.equal(addr2BalanceAfter + ethers.utils.parseEther("0.1").toBigInt());
  });
  ...
  it("Should redistribute ERC20 token", async() => {
    ...
    expect(await testToken.balanceOf(addr1.address)).to.be.equal(ethers.utils.parseEther("20"));
    expect(await testToken.balanceOf(addr2.address)).to.be.equal(ethers.utils.parseEther("80"));
    ...
    expect(await testToken.balanceOf(xlaRSCValve.address)).to.be.equal(0);
    expect(await testToken.balanceOf(addr1.address)).to.be.equal(ethers.utils.parseEther("40"));
    expect(await testToken.balanceOf(addr2.address)).to.be.equal(ethers.utils.parseEther("160"));

    await expect(
        xlaRSCValve.renounceOwnership()
    ).to.be.revertedWithCustomError(xlaRSCValve, "RenounceOwnershipForbidden")
  });
  ...
  it("Should create manual distribution split", async() => {
    ...
    const contractBalance = (await ethers.provider.getBalance(xlaRSCValveManualDistribution.address)).toBigInt();
    ...
    const contractBalance2 = (await ethers.provider.getBalance(xlaRSCValveManualDistribution.address)).toBigInt();
    expect(contractBalance2).to.be.equal(0);

    const addr1BalanceAfter = (await ethers.provider.getBalance(addr1.address)).toBigInt();
    expect(addr1BalanceAfter).to.be.equal(addr1BalanceBefore + ethers.utils.parseEther("25").toBigInt());
  });

  it("Should work with fees Correctly", async() => {
    const XLARSCValveFeeFactory = await ethers.getContractFactory("XLARSCValveFactory");
    ...
    ).to.be.revertedWithCustomError(xlaRSCValveFeeFactory, "InvalidFeePercentage");
    ...
    expect(await xlaRSCValveFeeFactory.platformWallet()).to.be.equal(addr5.address);
    expect(await xlaRSCValveFeeFactory.platformFee()).to.be.equal(BigInt(5000000));
    ...
    const revenueShareContractAddress = receipt.events?.[3].args?.[0];
    const XLARevenueShareContract = await ethers.getContractFactory("XLARSCValve");
    const xlaRSCFeeValve = await XLARevenueShareContract.attach(revenueShareContractAddress);

    const platformWalletBalanceBefore = (await ethers.provider.getBalance(addr5.address)).toBigInt()
    const addr1BalanceBefore = (await ethers.provider.getBalance(addr1.address)).toBigInt()
    ...
    const platformWalletBalanceAfter = (await ethers.provider.getBalance(addr5.address)).toBigInt()
    const addr1BalanceAfter = (await ethers.provider.getBalance(addr1.address)).toBigInt()
    ...
    await testToken.mint(xlaRSCFeeValve.address, ethers.utils.parseEther("100"))
    await xlaRSCFeeValve.redistributeToken(testToken.address)

    expect(await testToken.balanceOf(addr5.address)).to.be.equal(ethers.utils.parseEther("50"))
    expect(await testToken.balanceOf(addr1.address)).to.be.equal(ethers.utils.parseEther("50"))
  });

  it("Should work with creation ID correctly", async() => {
    const XLARSCValveCreationIdFactory = await ethers.getContractFactory("XLARSCValveFactory");
    const xlaRSCValveCreationIdFactory = await XLARSCValveCreationIdFactory.deploy();
    ...
    await expect(xlaRSCValveCreationIdFactory.createRSCValve(
        {
          controller: owner.address,
          distributors: [owner.address, ],
          isImmutableRecipients: true,
  ...
  it("Should recursively erc20 split", async() => {
    ...
    await testToken.mint(rscValveMain.address, ethers.utils.parseEther("1000000"));
    await testToken.mint(rscValveSecond.address, ethers.utils.parseEther("1000000"));
    await testToken.mint(rscValveThird.address, ethers.utils.parseEther("1000000"));
  ...
  it("Should recursively ETH split", async() => {
    ...
    expect(await ethers.provider.getBalance(rscValveMain.address)).to.be.equal(0);
    expect(await ethers.provider.getBalance(rscValveSecond.address)).to.be.equal(0);
    expect(await ethers.provider.getBalance(rscValveThird.address)).to.be.equal(0);
  });
  ...
  it("Should distribute small amounts correctly", async() => {
    ...
    await xlaRSCValve.redistributeToken(testToken.address);
    expect(await testToken.balanceOf(addr1.address)).to.be.equal(BigInt(3000000));
    expect(await testToken.balanceOf(addr2.address)).to.be.equal(BigInt(12000000));
    expect(await testToken.balanceOf(xlaRSCValve.address)).to.be.equal(BigInt(0));

    await testToken.mint(xlaRSCValve.address, BigInt(15000000));

    await xlaRSCValve.redistributeToken(testToken.address);
    expect(await testToken.balanceOf(addr1.address)).to.be.equal(BigInt(6000000));
    expect(await testToken.balanceOf(addr2.address)).to.be.equal(BigInt(24000000));
    expect(await testToken.balanceOf(xlaRSCValve.address)).to.be.equal(BigInt(0));
  });
  ...
  it("Should distribute small ether amounts correctly", async() => {
    ...
    const addr1BalanceBefore1 = (await ethers.provider.getBalance(addr1.address)).toBigInt()
    const addr2BalanceBefore1 = (await ethers.provider.getBalance(addr2.address)).toBigInt()
    ...
    expect(
        ...
        addr1BalanceBefore1 + ethers.utils.parseEther("0.0000000000075").toBigInt()
    );
    expect(
        (await ethers.provider.getBalance(addr2.address)).toBigInt()
    ).to.be.equal(
        addr2BalanceBefore1 + ethers.utils.parseEther("0.0000000000075").toBigInt()
    );
    expect((await ethers.provider.getBalance(rscValveXYZ.address)).toBigInt()).to.be.equal(BigInt(0));

    const addr1BalanceBefore2 = (await ethers.provider.getBalance(addr1.address)).toBigInt()
    const addr2BalanceBefore2 = (await ethers.provider.getBalance(addr2.address)).toBigInt()
    ...
    expect(
        ...
        addr1BalanceBefore2 + ethers.utils.parseEther("0.0000000000075").toBigInt()
    );

    expect(
        (await ethers.provider.getBalance(addr2.address)).toBigInt()
    ).to.be.equal(
        addr2BalanceBefore2 + ethers.utils.parseEther("0.0000000000075").toBigInt()
    );
  });
